...
Page({
  ...
  onShow() {
    // 每次进入刷新，防止其他页面改了资�?    this.loadProfile()
  },

  loadProfile() {
    const userInfo = wx.getStorageSync('userInfo') || {}
    const genderMap = { 0: '未设�?, 1: '�?, 2: '�? }
    this.setData({
      form: {
        id: userInfo.id || '',
      ...
      genderText: genderMap[userInfo.gender] || '未设�?
    })
  },
  ...
  // 保存（本�?+ 后端同步�?  saveProfile() {
    if (this.data.saving) return
    const form = this.data.form
    if (!form.nickName || !form.nickName.trim()) {
      wx.showToast({ title: '请设置昵�?, icon: 'none' })
      return
    }
    this.setData({ saving: true })
    const nickname = form.nickName.trim()

    // 1. 先更新本�?    const userInfo = {
    ...
    app.globalData.request.put('/user/info', {
      ...
      this.setData({ saving: false })
      // 即使后端失败，本地已保存，提示用�?      wx.showToast({ title: '本地已保存，后端同步失败', icon: 'none' })
      setTimeout(() => wx.navigateBack(), 800)
    })
  }
})
